...
fun Fragment.shareFile(file: File, subject: String? = null, text: String? = null) {
    try {
        ...
        intent.type = "*/*"
        intent.putExtra(Intent.EXTRA_STREAM, FileProvider.getUriForFile(requireContext(), "com.refah.superapp.fileprovider", file))
...
fun Fragment.copyText(text: String? = null) {
    try {
        ...
        ir.drax.extensions.message(R.string.copy_done)
...
fun Fragment.message(message: Int, actionText: String = "", callback: View.OnClickListener? = null) {
    ir.drax.extensions.message(getString(message), actionText, callback)
}
