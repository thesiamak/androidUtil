...
/**
 * WaterfallExpandable
 * Just like normal expandable it can hide and show views
 * The difference is the animation and implementation method
 * If you have lists views(Listview ,RecyclerView) We recommend
 * using normal expandable due to a layout bug.Which does not
 * guaranty a layout refresh when on hidden state. But everything
 * seem OK on shown state so far.
 * In case you are looking for a lighter implementation &
 * waterfall animation & Smoother layout change this is a
 * better choice.
 * */
class WaterfallExpandable @JvmOverloads constructor(context: Context, attrs:AttributeSet?=null, defStyleAttr:Int=0)  : ConstraintLayout(context,attrs,defStyleAttr), Observer {
    ...
    private fun enable(){
        ...
        enable(collapsedHeight,expandedHeight)
    }
    private fun enable(from:Int,to:Int){
        ...
        resizeAnimation=ResizeAnimation(this,to,from)
        resizeAnimation?.let{
            it.duration = EXPAND_DURATION.toLong()
            it.startOffset = 200
    ...
    fun addChild(vararg views:View ):WaterfallExpandable{
        childs = childs + views
